(function() {
    ...
    function showLobby() { lobbyScreen.style.display = 'flex'; }
    function hideLobby() { lobbyScreen.style.display = 'none'; }
    function showGame() { document.querySelectorAll('#gameCanvas, #hud, #top-bar, #hotbar, #command-box').forEach(el => el.style.display = el.tagName==='CANVAS'?'block':(el.id==='top-bar'?'flex':'flex')); }
    function hideGame() { document.querySelectorAll('#gameCanvas, #hud, #top-bar, #hotbar, #command-box, #hint-bubble, #notification, #levelup-modal, #shop-modal, #editor-modal, #friends-modal, #water-bar-container').forEach(el => { if(el) el.style.display = 'none'; }); }
    ...
    function startPeer(room) {
        ...
        document.getElementById('multiplayer-status').textContent = '🟡 Conectando...';

        peer = new Peer('codegarden_' + room + '_' + (isHost ? 'host' : 'guest'), { debug: 0 });

        peer.on('open', (id) => {
            myPeerId = id;
            document.getElementById('multiplayer-status').textContent = isHost ? '🏠 Host' : '🔗 Convidado';
            document.getElementById('multiplayer-status').style.color = '#7cfc00';
            startGame();

            if (!isHost) {
                conn = peer.connect('codegarden_' + room + '_host');
                conn.on('open', () => { showNotification('🔗 Conectado ao host!'); setupConnection(conn); });
            }
        });

        peer.on('connection', (incomingConn) => {
            if (isHost) { conn = incomingConn; setupConnection(conn); showNotification('👤 Jogador entrou na sala!'); }
        });

        peer.on('error', (err) => {
            console.error('Peer error:', err);
            document.getElementById('multiplayer-status').textContent = '❌ Erro';
            document.getElementById('multiplayer-status').style.color = '#ff6b6b';
        });
    }
    ...
    function setupConnection(connection) {
        ...
        conn.on('close', () => {
            ...
            document.getElementById('multiplayer-status').textContent = '⚫ Desconectado';
            document.getElementById('multiplayer-status').style.color = '#888';
        });
    }

    function broadcastPosition() {
        if (conn && conn.open) {
            conn.send({ type: 'position', id: myPeerId, x: player.x, y: player.y, tileX: player.tileX, tileY: player.tileY, name: playerName });
        }
    }

    function broadcastAction(actionData) {
        if (conn && conn.open) conn.send({ type: 'action', ...actionData });
    }
    ...
    loginBtn.addEventListener('click', () => {
        ...
        hideLogin();
        showLobby();
        document.getElementById('room-id-input').value = 'garden_' + Math.random().toString(36).slice(2, 8);
    });

    googleBtn.addEventListener('click', () => {
        const fakeGoogleEmails = ['fazendeiro.dev@gmail.com', 'code.garden@gmail.com'];
        const randomEmail = fakeGoogleEmails[Math.floor(Math.random() * fakeGoogleEmails.length)];
        playerName = randomEmail.split('@')[0].replace(/[^a-zA-Z0-9]/g, '').slice(0, 14);
        localStorage.setItem('codegarden_username', playerName);
        hideLogin();
        showLobby();
    });

    document.getElementById('register-link').addEventListener('click', () => { loginEmail.value = ''; loginPassword.value = ''; loginEmail.focus(); showLoginError('Use qualquer email e senha!'); });
    loginEmail.addEventListener('keydown', (e) => { if (e.key === 'Enter') loginPassword.focus(); });
    loginPassword.addEventListener('keydown', (e) => { if (e.key === 'Enter') loginBtn.click(); });

    if (savedName) { playerName = savedName; hideLogin(); showLobby(); document.getElementById('room-id-input').value = 'garden_' + Math.random().toString(36).slice(2, 8); }

    initMultiplayer();

    function getFriends() { const data = localStorage.getItem('codegarden_friends_' + playerName); return data ? JSON.parse(data) : []; }
    function saveFriends(friends) { localStorage.setItem('codegarden_friends_' + playerName, JSON.stringify(friends)); }
    function renderFriends() { const list = document.getElementById('friends-list'); const friends = getFriends(); list.innerHTML = friends.length === 0 ? '<div style="color:#888;font-size:7px;padding:10px;">Nenhum amigo ainda.</div>' : friends.map(f => `<div class="friend-row"><span>🧑‍🌾 ${f.name}</span><span class="${Math.random() > 0.5 ? 'online' : 'offline'}">${Math.random() > 0.5 ? '🟢 Online' : '⚫ Offline'}</span><button class="btn" style="font-size:6px;padding:4px 6px;" data-remove="${f.name}">✖</button></div>`).join(''); list.querySelectorAll('[data-remove]').forEach(btn => { btn.addEventListener('click', (e) => { saveFriends(getFriends().filter(f => f.name !== e.target.getAttribute('data-remove'))); renderFriends(); }); }); }
    ...
    document.getElementById('add-friend-btn').addEventListener('click', () => { const input = document.getElementById('friend-input'); const name = input.value.trim(); if (name.length < 2) return; const friends = getFriends(); if (friends.find(f => f.name === name)) { showNotification('❌ Já está na lista!'); return; } friends.push({ name, online: true }); saveFriends(friends); renderFriends(); input.value = ''; showNotification(`👥 ${name} adicionado!`); });
    ...
    function initGame() {
        ...
        window.farm = {
            soil: { till: () => { if (selectedTool !== 'hoe') { showNotification('❌ Equipe TOOLS.HOE primeiro.'); return false; } const x = player.tileX, y = player.tileY; if (map[y][x] === TILE_GRASS) { map[y][x] = TILE_TILLED; showNotification('⛏️ Terra arada!'); broadcastAction({ action: 'till', tileX: x, tileY: y }); return true; } showNotification('❌ Só pode arar grama.'); return false; }, isTilled: () => { const x = player.tileX, y = player.tileY; return map[y][x] === TILE_TILLED; } },
            crops: { plant: (crop) => { const x = player.tileX, y = player.tileY; if (map[y][x] !== TILE_TILLED) { showNotification('❌ Precisa arar primeiro.'); return false; } if (crops[`${x},${y}`]) { showNotification('❌ Já tem planta aqui.'); return false; } const cropIndex = Object.keys(CROP_NAMES).indexOf(crop); if (cropIndex === -1) { showNotification('❌ Use CROPS.WHEAT, etc.'); return false; } selectedCrop = cropIndex; selectedTool = null; updateHotbar(); crops[`${x},${y}`] = { type: cropIndex, timer: 0, ready: false, growTime: Object.values(CROP_GROW_TIMES)[cropIndex] }; showNotification(`🌱 Plantou ${CROP_NAMES[crop]}!`); broadcastAction({ action: 'plant', tileX: x, tileY: y, cropType: cropIndex }); return true; }, water: () => { if (!hasWateringCan) { showNotification('❌ Precisa do regador (Nv.3).'); return false; } if (playerWater <= 0) { showNotification('💧 Regador vazio!'); return false; } const x = player.tileX, y = player.tileY, k = `${x},${y}`; if (crops[k] && !crops[k].ready) { crops[k].timer += 30; playerWater--; updateWaterBar(); showNotification('💦 Regou!'); broadcastAction({ action: 'water', tileX: x, tileY: y }); return true; } showNotification('❌ Nada para regar aqui.'); return false; }, harvest: () => { const x = player.tileX, y = player.tileY, k = `${x},${y}`; if (crops[k] && crops[k].ready) { playerXP += 20; updateLevel(); delete crops[k]; playerCoins += 10; showNotification('🌾 Colheu! +20 XP +10💰'); broadcastAction({ action: 'harvest', tileX: x, tileY: y }); return true; } if (map[y][x] === TILE_FRUIT_TREE && fruitTreeTimers[k] >= 300) { playerXP += 50; updateLevel(); fruitTreeTimers[k] = 0; playerCoins += 25; showNotification('🍎 Frutas! +50 XP +25💰'); return 'fruit'; } showNotification('❌ Nada pronto para colher.'); return false; }, isReady: () => { const x = player.tileX, y = player.tileY, k = `${x},${y}`; return crops[k] && crops[k].ready; } },
        ...
        function updateWaterBar() { const container = document.getElementById('water-bar-container'); if (hasWateringCan) { container.style.display = 'flex'; document.getElementById('water-bar-inner').style.width = (playerWater / MAX_WATER * 100) + '%'; document.getElementById('water-text').textContent = playerWater + '/' + MAX_WATER; } else { container.style.display = 'none'; } }
        ...
        function updateShopUI() { const lampItem = document.getElementById('shop-lamppost'); if (playerLevel >= 2) { lampItem.classList.remove('locked'); lampItem.innerHTML = '<div class="item-info"><span class="item-name">💡 Poste de Luz</span></div><button class="btn" data-item="lamppost">COMPRAR</button>'; } const wellItem = document.getElementById('shop-well'); if (playerLevel >= 3) { wellItem.classList.remove('locked'); wellItem.innerHTML = '<div class="item-info"><span class="item-name">🪣 Poço</span></div><button class="btn" data-item="well">COMPRAR</button>'; } const bootsItem = document.getElementById('shop-boots'); if (playerLevel >= 4) { bootsItem.classList.remove('locked'); bootsItem.innerHTML = '<div class="item-info"><span class="item-name">👢 Botas</span></div><button class="btn" data-item="boots">COMPRAR</button>'; } if (playerLevel >= 3 && !hasWateringCan) { hasWateringCan = true; playerWater = MAX_WATER; updateWaterBar(); document.getElementById('watering-can-slot').classList.remove('locked'); showNotification('🚿 Regador desbloqueado!'); } if (playerLevel >= 4 && !hasBoots) { hasBoots = true; document.getElementById('boots-slot').classList.remove('locked'); showNotification('👢 Botas desbloqueadas!'); } bindShopButtons(); }
        function bindShopButtons() { document.querySelectorAll('#shop-modal .btn[data-item]').forEach(btn => { const newBtn = btn.cloneNode(true); btn.parentNode.replaceChild(newBtn, btn); }); document.querySelectorAll('#shop-modal .btn[data-item]').forEach(btn => { btn.addEventListener('click', (e) => { farm.shop.buy(e.target.getAttribute('data-item')); document.getElementById('shop-modal').style.display = 'none'; }); }); }
        ...
        function showNotification(msg) { const el = document.getElementById('notification'); el.textContent = msg; el.style.opacity = '1'; clearTimeout(el._timeout); el._timeout = setTimeout(() => el.style.opacity = '0', 2500); }
        function showLevelUpModal() { const modal = document.getElementById('levelup-modal'); document.getElementById('levelup-level').textContent = `Nível ${playerLevel}`; playerCoins += playerLevel * 100; let unlocks = ''; if (playerLevel === 2) unlocks = '🔓 Poste de Luz!'; if (playerLevel === 3) unlocks = '🔓 Regador + Poço!'; if (playerLevel === 4) unlocks = '🔓 Botas!'; document.getElementById('levelup-unlock').textContent = unlocks; modal.style.display = 'block'; document.getElementById('levelup-close').onclick = () => { modal.style.display = 'none'; }; clearTimeout(modal._timeout); modal._timeout = setTimeout(() => { modal.style.display = 'none'; }, 5000); }
        ...
        function executeFreeCommand(cmd) { if (!cmd) return; try { eval(cmd); } catch (e) { showNotification('❓ Comando não reconhecido: ' + cmd); } }
        ...
        function checkNearbyInteraction() {
            const px = player.x, py = player.y, tileX = player.tileX, tileY = player.tileY, key = `${tileX},${tileY}`, tile = map[tileY] ? map[tileY][tileX] : null; hideHint();
            ...
            if (tile === TILE_TILLED && !crops[key] && selectedCrop >= 0 && selectedTool === null) { const cropName = Object.keys(CROP_NAMES)[selectedCrop]; showHint(formatHint(`farm.crops.plant(CROPS.${cropName.toUpperCase()})`, 'Terra arada', 'true se plantou')); return; }
            ...
            const nearestAnimal = wildAnimals.find(a => a.isInteracting && a.promptTimer > 30 && Math.hypot(px - a.x, py - a.y) < 1.5);
            if (nearestAnimal && selectedTool !== 'leash') { showHint(formatHint(`farm.animals.feed("${nearestAnimal.name}")`, 'Próximo ao animal', 'true se alimentado')); return; }
