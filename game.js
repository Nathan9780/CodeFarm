(function () {
  ...
  function initMultiplayer() {
    ...
    document.getElementById("host-btn").addEventListener("click", () => {
      ...
      setTimeout(() => startPeer(roomId), 500);
    });

    document.getElementById("join-btn").addEventListener("click", () => {
      roomId = roomInput.value.trim();
      if (!roomId) {
        lobbyInfo.textContent = "Digite o ID da sala!";
        return;
      }
      isHost = false;
      if (roomDisplay) roomDisplay.style.display = "none";
  ...
  function connectToHost(room) {
    ...
    if (connectionRetries > MAX_RETRIES) {
      document.getElementById("multiplayer-status").textContent =
        "❌ Sala não encontrada";
      document.getElementById("multiplayer-status").style.color = "#ff6b6b";
      showNotification("❌ Verifique o código da sala.");
      return;
    }
    ...
    possibleHostIds.forEach((hostId, index) => {
      if (connected) return;

      setTimeout(() => {
        if (connected) return;
        console.log("🔍 Tentando:", hostId);
        const testConn = peer.connect(hostId, { reliable: true });
  ...
  loginBtn.addEventListener("click", () => {
    ...
    if (email.length < 5 || !email.includes("@")) {
      showLoginError("Digite um email válido.");
      return;
    }
    if (password.length < 3) {
      showLoginError("A senha deve ter pelo menos 3 caracteres.");
      return;
    }
    playerName =
      email
  ...
  googleBtn.addEventListener("click", () => {
    ...
    playerName = emails[Math.floor(Math.random() * emails.length)]
      .split("@")[0]
      .replace(/[^a-zA-Z0-9]/g, "")
      .slice(0, 14);
    localStorage.setItem("codegarden_username", playerName);
  ...
  function saveFriends(friends) {
    localStorage.setItem(
      "codegarden_friends_" + playerName,
  ...
  document.getElementById("add-friend-btn").addEventListener("click", () => {
    ...
    if (friends.find((f) => f.name === name)) {
      showNotification("❌ Já está na lista!");
      return;
    }
  ...
  function initGame() {
    ...
    window.farm = {
      soil: {
        till: () => {
          if (selectedTool !== "hoe") {
            showNotification("❌ Equipe TOOLS.HOE primeiro.");
            return false;
          }
          ...
          showNotification("❌ Só pode arar grama.");
          return false;
        },
        isTilled: () =>
      ...
      crops: {
        plant: (crop) => {
          ...
          if (map[y][x] !== TILE_TILLED) {
            showNotification("❌ Precisa arar primeiro.");
            return false;
          }
          if (crops[`${x},${y}`]) {
            showNotification("❌ Já tem planta aqui.");
            return false;
          }
        ...
        water: () => {
          if (!hasWateringCan) {
            showNotification("❌ Precisa do regador (Nv.3).");
            return false;
          }
          if (playerWater <= 0) {
            showNotification("💧 Regador vazio!");
            return false;
          }
          const x = player.tileX,
            y = player.tileY,
          ...
          showNotification("❌ Nada para regar aqui.");
          return false;
        },
        harvest: () => {
          const x = player.tileX,
          ...
          if (crops[k] && crops[k].ready) {
            ...
            delete crops[k];
            playerCoins += 10;
            showNotification("🌾 Colheu! +20 XP +10💰");
          ...
          if (map[y][x] === TILE_FRUIT_TREE && fruitTreeTimers[k] >= 300) {
            ...
            fruitTreeTimers[k] = 0;
            playerCoins += 25;
            showNotification("🍎 Frutas! +50 XP +25💰");
            return "fruit";
          }
          showNotification("❌ Nada pronto para colher.");
          return false;
        },
        isReady: () => {
      ...
      animals: {
        feed: (name) => {
          const nearest = wildAnimals.find(
            (a) =>
              a.name === name &&
          ...
          nearest.fed = true;
          nearest.feedCount++;
          spawnHearts(nearest.x, nearest.y, 4);
          showNotification(`🐾 ${nearest.name} alimentado! +${xpG} XP`);
          if (nearest.feedCount >= 2 && nearest.stage < 2) {
            nearest.stage++;
            nearest.feedCount = 0;
            nearest.growthStart =
              Date.now() -
              (nearest.stage === 1 ? GROWTH_STAGE1 : GROWTH_STAGE2);
            showNotification(`🌟 ${nearest.name} cresceu!`);
            spawnHearts(nearest.x, nearest.y, 10);
          }
          return true;
        },
        lead: () => {
      ...
      buildings: {
        place: () => {
          if (!pendingBuilding) {
            showNotification("❌ Compre algo primeiro.");
            return false;
          }
      ...
      tools: {
        ...
        getWater: () => {
          ...
          if (nw || nwb) {
            playerWater = MAX_WATER;
            updateWaterBar();
            showNotification("💧 Regador cheio!");
            return true;
          }
          return false;
        },
      ...
      shop: {
        buy: (item) => {
          if (["barn", "silo", "fence", "lamppost", "well"].includes(item)) {
            ...
            showNotification(`🏗️ ${item} comprado! Use farm.buildings.place()`);
            return true;
          }
          if (item === "boots") {
            if (playerLevel < 4) {
          ...
          if (
            ...
            pendingAnimal = item;
            showNotification(
              `🐾 ${SPECIES_NAMES[item]} comprado! Clique no mapa.`,
            );
            return true;
          }
          return false;
    ...
    function skipToNextPeriod() {
      ...
      showNotification("💤 Você descansou.");
    }
    ...
    function formatHint(func, condition, returnInfo) {
      return `${func}<br><span class="hint-detail">${condition ? "Cond: " + condition + ". " : ""}Ret: ${returnInfo}</span>`;
    }
    ...
    function update() {
      ...
      if (player.moving) {
        const e = performance.now() - player.moveStartTime,
          p = Math.min(e / MOVE_DURATION, 1.0);
        player.x =
    ...
    function draw() {
      ...
      wildAnimals.forEach((a) => {
        const px = (a.x - camX) * TILE_SIZE,
          ...
          sizeScale = [0.5, 0.75, 1.0][a.stage];
